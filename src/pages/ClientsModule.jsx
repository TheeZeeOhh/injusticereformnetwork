...
export default function ClientsModule() {
  ...
  const DEFAULT_SMS = t('clients.smsDefault');
    ...
    async function loadActiveClient() {
      ...
      try {
        ...
        if (stored) {
          ...
          // New blank client
          setClientData({ legalName: t('clients.newClient'), alias: '', phone: '', emergency: '', smsConsent: false, photo: '', pronouns: [], pronounsSelfDescribe: '' });
        }
        setSmsStatus(null);
  ...
  const handleSaveIntake = async () => {
    ...
    try {
      ...
      setIntakeStatus({ ok: true, msg: t('clients.intakeSaved') });
    } catch (err) {
  ...
  const handleSendReminder = async () => {
    if (!clientData.smsConsent) {
      setSmsStatus({ ok: false, msg: t('clients.smsNoConsent') });
      return;
    }
    const to = (clientData.phone || '').trim();
    if (!to) {
      setSmsStatus({ ok: false, msg: t('clients.smsNoContact') });
      return;
    }
    if (!smsBody.trim()) {
      setSmsStatus({ ok: false, msg: t('clients.smsEmpty') });
      return;
    }
    ...
    try {
      ...
      setSmsStatus({ ok: false, msg: typeof err === 'string' ? err : (err?.message || t('clients.smsSendFailed')) });
    }
    setSmsSending(false);
  };
    ...
    return (
      <div style={{ height: '100%', display: 'flex', flexDirection: 'column', gap: '2rem' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <h1 style={{ margin: 0, color: 'var(--gold)', fontFamily: 'var(--font-serif)' }}>{t('clients.directory')}</h1>
            <p style={{ margin: 0, color: 'var(--text-secondary)', fontFamily: 'var(--font-mono)' }}>{t('clients.directorySub')}</p>
          </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontFamily: 'var(--font-mono)' }}>
            <thead>
              <tr style={{ borderBottom: '1px solid var(--border-color)', color: 'var(--text-secondary)', textAlign: 'left' }}>
                <th style={{ padding: '1rem' }}>{t('clients.colClientId')}</th>
                <th style={{ padding: '1rem' }}>{t('clients.colLegalName')}</th>
                <th style={{ padding: '1rem' }}>{t('clients.colStatus')}</th>
                <th style={{ padding: '1rem' }}>{t('clients.colAction')}</th>
              </tr>
            </thead>
            <tbody>
      ...
      <div className="glass-panel" style={{ flex: 1, padding: '2rem', overflowY: 'auto' }}>
        ...
        {activeTab === 'profile' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
            <h2 style={{ color: 'var(--gold)', borderBottom: '1px solid var(--border-color)', paddingBottom: '0.5rem', margin: 0, fontFamily: 'var(--font-serif)' }}>{t('clients.profile')}</h2>
            ...
            <div>
              ...
              <input
                ...
                placeholder={t('clients.pronounsSelfDescribe')}
            ...
            <div style={{ borderTop: '1px solid var(--border-color)', paddingTop: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              <h3 style={{ margin: 0, color: 'var(--gold)', fontFamily: 'var(--font-serif)', fontSize: '1rem' }}>{t('clients.smsReminder')}</h3>
              ...
              <div>
                <label style={{ display: 'block', fontSize: '0.8rem', color: 'var(--text-secondary)', marginBottom: '0.25rem' }}>{t('clients.message')}</label>
                <textarea
                  value={smsBody}
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                ...
                >
                  {smsSending ? t('clients.sending') : t('clients.sendReminder')}
                </button>
                {smsStatus && (
        ...
        {activeTab === 'housing' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
            <h2 style={{ color: 'var(--gold)', borderBottom: '1px solid var(--border-color)', paddingBottom: '0.5rem', margin: 0, fontFamily: 'var(--font-serif)' }}>{t('clients.housingTracker')}</h2>
            <p style={{ color: 'var(--text-secondary)', fontSize: '0.9rem', fontFamily: 'var(--font-mono)' }}>{t('clients.currentStatus')} <strong style={{ color: 'var(--ember)' }}>{t('clients.pendingVouchers')}</strong></p>
          </div>
        )}
        ...
        {activeTab === 'canvas' && (
          <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
            <h2 style={{ color: 'var(--gold)', margin: '0 0 1rem', fontFamily: 'var(--font-serif)' }}>{t('clients.canvas')}</h2>
            <div style={{ flex: 1 }}>
